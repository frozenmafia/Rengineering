...
final class AudioTrackPositionTracker {
    ...
    public long getCurrentPositionUs(boolean z) {
        ...
        if (j2 < 1000000) {
            long j3 = this.previousModePositionUs;
            long mediaDurationForPlayoutDuration = Util.getMediaDurationForPlayoutDuration(j2, this.audioTrackPlaybackSpeed);
            long j4 = (j2 * 1000) / 1000000;
            j = ((j * j4) + ((1000 - j4) * (j3 + mediaDurationForPlayoutDuration))) / 1000;
        }
        if (!this.notifiedPositionIncreasing) {
            long j5 = this.lastPositionUs;
            if (j > j5) {
                this.notifiedPositionIncreasing = true;
                long playoutDurationForMediaDuration = Util.getPlayoutDurationForMediaDuration(Util.usToMs(j - j5), this.audioTrackPlaybackSpeed);
                this.listener.onPositionAdvancing(System.currentTimeMillis() - Util.usToMs(playoutDurationForMediaDuration));
            }
        }
