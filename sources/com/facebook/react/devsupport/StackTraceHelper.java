...
public class StackTraceHelper {
    ...
    public static class StackFrameImpl implements StackFrame {
        ...
        public JSONObject toJSON() {
            return new JSONObject(destroyItem.ag$a(StringSet.file, getFile(), "methodName", getMethod(), StackTraceHelper.LINE_NUMBER_KEY, Integer.valueOf(getLine()), StackTraceHelper.COLUMN_KEY, Integer.valueOf(getColumn()), "collapse", Boolean.valueOf(isCollapsed())));
        }
    }
    ...
    public static StackFrame[] convertJsStackTrace(ReadableArray readableArray) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (type == ReadableType.Map) {
                ...
                stackFrameArr[i] = new StackFrameImpl(map.getString(StringSet.file), string, (!map.hasKey(LINE_NUMBER_KEY) || map.isNull(LINE_NUMBER_KEY)) ? -1 : map.getInt(LINE_NUMBER_KEY), (!map.hasKey(COLUMN_KEY) || map.isNull(COLUMN_KEY)) ? -1 : map.getInt(COLUMN_KEY), map.hasKey("collapse") && !map.isNull("collapse") && map.getBoolean("collapse"));
    ...
    public static StackFrame[] convertJsStackTrace(JSONArray jSONArray) {
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                stackFrameArr[i] = new StackFrameImpl(jSONObject.getString(StringSet.file), jSONObject.getString("methodName"), (!jSONObject.has(LINE_NUMBER_KEY) || jSONObject.isNull(LINE_NUMBER_KEY)) ? -1 : jSONObject.getInt(LINE_NUMBER_KEY), (!jSONObject.has(COLUMN_KEY) || jSONObject.isNull(COLUMN_KEY)) ? -1 : jSONObject.getInt(COLUMN_KEY), jSONObject.has("collapse") && !jSONObject.isNull("collapse") && jSONObject.getBoolean("collapse"));
