...
public class Debug {
    public static void logStack(String str, String str2, int i) {
        ...
        String str3 = Constants.WHITE_SPACE;
        for (int i2 = 1; i2 <= min; i2++) {
            StackTraceElement stackTraceElement = stackTrace[i2];
            str3 = str3 + Constants.WHITE_SPACE;
    ...
    public static void printStack(String str, int i) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        int min = Math.min(i, stackTrace.length - 1);
        String str2 = Constants.WHITE_SPACE;
        for (int i2 = 1; i2 <= min; i2++) {
            StackTraceElement stackTraceElement = stackTrace[i2];
            str2 = str2 + Constants.WHITE_SPACE;
        ...
        for (Field field : cls.getFields()) {
            try {
                ...
                if (field.getName().startsWith("layout_constraint") && ((!(obj2 instanceof Integer) || !obj2.toString().equals("-1")) && ((!(obj2 instanceof Integer) || !obj2.toString().equals(SessionDescription.SUPPORTED_SDP_VERSION)) && ((!(obj2 instanceof Float) || !obj2.toString().equals("1.0")) && (!(obj2 instanceof Float) || !obj2.toString().equals("0.5")))))) {
                    System.out.println(str + "    " + field.getName() + Constants.WHITE_SPACE + obj2);
                }
    ...
    public static String getName(Context context, int[] iArr) {
        ...
        try {
            ...
            while (i < iArr.length) {
                ...
                sb.append(str2);
                sb.append(i == 0 ? "" : Constants.WHITE_SPACE);
                String sb2 = sb.toString();
                try {
                    str = context.getResources().getResourceEntryName(iArr[i]);
                } catch (Resources.NotFoundException unused) {
                    str = "? " + iArr[i] + Constants.WHITE_SPACE;
                }
                str2 = sb2 + str;
                i++;
            }
            ...
            for (Field field : layoutParams.getClass().getFields()) {
                try {
                    ...
                    if (field.getName().contains("To") && !obj.toString().equals("-1")) {
                        System.out.println(str2 + "       " + field.getName() + Constants.WHITE_SPACE + obj);
                    }
                } catch (IllegalAccessException unused) {
                }
            }
        ...
        for (Field field : layoutParams.getClass().getFields()) {
            try {
                ...
                if (name.contains("To") && !obj.toString().equals("-1")) {
                    System.out.println(str2 + "       " + name + Constants.WHITE_SPACE + obj);
                }
            } catch (IllegalAccessException unused) {
            }
        }
