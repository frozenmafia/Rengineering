...
public abstract class AbstractResolvableFuture<V> implements animateAddImpl<V> {
    ...
    static {
        ...
        try {
            synchronizedHelper = new SafeAtomicHelper(AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Thread.class, "thread"), AtomicReferenceFieldUpdater.newUpdater(Waiter.class, Waiter.class, StringSet.next), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Waiter.class, "waiters"), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Listener.class, "listeners"), AtomicReferenceFieldUpdater.newUpdater(AbstractResolvableFuture.class, Object.class, "value"));
            th = null;
        } catch (Throwable th) {
            th = th;
    ...
    public static final class SetFuture<V> implements Runnable {
        final animateAddImpl<? extends V> future;
        final AbstractResolvableFuture<V> owner;

        SetFuture(AbstractResolvableFuture<V> abstractResolvableFuture, animateAddImpl<? extends V> animateaddimpl) {
            this.owner = abstractResolvableFuture;
            this.future = animateaddimpl;
        }
    ...
    public final V get(long j, TimeUnit timeUnit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        String str = "Waited " + j + com.dreampay.commons.constants.Constants.WHITE_SPACE + timeUnit.toString().toLowerCase(Locale.ROOT);
        if (nanos + 1000 < 0) {
            ...
            if (i > 0) {
                String str3 = str2 + convert + com.dreampay.commons.constants.Constants.WHITE_SPACE + lowerCase;
                if (z) {
                    str3 = str3 + ",";
                }
                str2 = str3 + com.dreampay.commons.constants.Constants.WHITE_SPACE;
            }
            if (z) {
    ...
    */
    public final boolean cancel(boolean r8) {
        ...
        L35:
            ...
            o.animateAddImpl<? extends V> r0 = r0.future
    ...
    public boolean setFuture(animateAddImpl<? extends V> animateaddimpl) {
        Failure failure;
        checkNotNull(animateaddimpl);
        Object obj = this.value;
        if (obj == null) {
            if (animateaddimpl.isDone()) {
                if (ATOMIC_HELPER.casValue(this, null, getFutureValue(animateaddimpl))) {
                    complete(this);
                    return true;
                }
                return false;
            }
            SetFuture setFuture = new SetFuture(this, animateaddimpl);
            if (ATOMIC_HELPER.casValue(this, null, setFuture)) {
                try {
                    animateaddimpl.addListener(setFuture, DirectExecutor.INSTANCE);
                } catch (Throwable th) {
                    try {
        ...
        if (obj instanceof Cancellation) {
            animateaddimpl.cancel(((Cancellation) obj).wasInterrupted);
        }
        return false;
    }

    static Object getFutureValue(animateAddImpl<?> animateaddimpl) {
        if (animateaddimpl instanceof AbstractResolvableFuture) {
            Object obj = ((AbstractResolvableFuture) animateaddimpl).value;
            if (obj instanceof Cancellation) {
        ...
        boolean isCancelled = animateaddimpl.isCancelled();
        if ((!GENERATE_CANCELLATION_CAUSES) & isCancelled) {
            return Cancellation.CAUSELESS_CANCELLED;
        }
        try {
            Object uninterruptibly = getUninterruptibly(animateaddimpl);
            return uninterruptibly == null ? NULL : uninterruptibly;
        } catch (CancellationException e) {
            if (!isCancelled) {
                return new Failure(new IllegalArgumentException("get() threw CancellationException, despite reporting isCancelled() == false: " + animateaddimpl, e));
            }
