...
public class VelocityMatrix {
    ...
    public void applyTransform(float f, float f2, int i, int i2, float[] fArr) {
        ...
        double d = i2 * f6;
        fArr[0] = f4 + f7 + (f9 * (f - 0.5f) * 2.0f) + (((float) ((((-i) * f3) * Math.sin(radians2)) - (Math.cos(radians2) * d))) * radians);
        fArr[1] = f5 + f8 + (f10 * f6) + (radians * ((float) (((i * f3) * Math.cos(radians2)) - (d * Math.sin(radians2)))));
    }
}
