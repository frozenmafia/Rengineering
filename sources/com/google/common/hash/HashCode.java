...
public abstract class HashCode {
    ...
    public int writeBytesTo(byte[] bArr, int i, int i2) {
        int valueOf = Ints.valueOf(i2, bits() / 8);
        markViewHoldersUpdated.ag$a(i, i + valueOf, bArr.length);
        writeBytesToImpl(bArr, i, valueOf);
        return valueOf;
    }
    ...
    static final class IntHashCode extends HashCode implements Serializable {
        ...
        public long padToLong() {
            return UnsignedInts.toString(this.hash);
        }
    ...
    public static HashCode fromBytes(byte[] bArr) {
        markViewHoldersUpdated.toString(bArr.length >= 1, "A HashCode must contain at least 1 byte.");
    ...
    public static final class BytesHashCode extends HashCode implements Serializable {
        ...
        BytesHashCode(byte[] bArr) {
            this.bytes = (byte[]) markViewHoldersUpdated.toString(bArr);
        }
        ...
        public int asInt() {
            ...
            markViewHoldersUpdated.valueOf(bArr.length >= 4, "HashCode#asInt() requires >= 4 bytes (it only has %s bytes).", bArr.length);
        ...
        public long asLong() {
            byte[] bArr = this.bytes;
            markViewHoldersUpdated.valueOf(bArr.length >= 8, "HashCode#asLong() requires >= 8 bytes (it only has %s bytes).", bArr.length);
            return padToLong();
        }
    ...
    public static HashCode fromString(String str) {
        markViewHoldersUpdated.toString(str.length() >= 2, "input string (%s) must have at least 2 characters", str);
        markViewHoldersUpdated.toString(str.length() % 2 == 0, "input string (%s) must have an even number of characters", str);
